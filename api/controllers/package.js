...
function packages(req, res) {
  res.json(['onlypackage']);
}

function getPackage(req, res) {
	var name = req.swagger.params.contractName.value || 'onlypackage';
  ...
  contractContract.getPackageTemperature(function(error, result) {
    if(error) console.log(error);
      ...
      console.log(result);
      res.json({packageName: name, packageTemperature: result.toNumber()});
    }
  });
}

function putPackage(req, res) {
    var packageToBeUpdated = req.body;
    var temperatureToSet = packageToBeUpdated.packageTemperature;
    ...
    contractContract.setPackageTemperature(temperatureToSet, function(error, result) {
      if(error) console.log(error);
        ...
        res.json({message: packageToBeUpdated.packageTemperature + ' set!'});
      }
    });
}
